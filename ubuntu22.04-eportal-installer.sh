...
# Simple password function that works reliably
read_password() {
    local password=""
    local prompt="$1"
    ...
    # Simple approach - just use read -s and tell user password is hidden
    read -s password
...
check_apt_repo() {
    ...
    if [[ -f "$repo_file" ]]; then
        if grep -q "$expected_url" "$repo_file" && grep -q "jammy main" "$repo_file"; then
    ...
    if [[ -f /etc/os-release ]]; then
        ...
        if [[ "$ID" == "ubuntu" ]]; then
            if [[ "$VERSION_ID" == "22.04" ]]; then
                log_message "Detected Ubuntu 22.04. Proceeding with installation..."
                return 0
            else
                log_message "WARNING: This script is designed for Ubuntu 22.04, but detected $ID $VERSION_ID."
            ...
            log_message "ERROR: This script is designed for Ubuntu 22.04, but detected $ID $VERSION_ID."
            exit 1
        fi
    else
...
clear
log_message "Starting TuxCare ePortal installation for Ubuntu 22.04..."
echo
...
if ! check_apt_repo "/etc/apt/sources.list.d/kcare-eportal.list" "https://repo.cloudlinux.com/kcare-eportal/22.04" "TuxCare ePortal"; then
