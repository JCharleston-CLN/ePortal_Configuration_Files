#!/bin/bash
# ****************************************************************************
#  Title: TuxCare Ubuntu 24.04 ePortal Installer script.
#  Purpose: Install and configure TuxCare ePortal on Ubuntu 24.04.
...
check_apt_repo() {
    ...
    if [[ -f "$repo_file" ]]; then
        if grep -q "$expected_url" "$repo_file" && grep -q "noble main" "$repo_file"; then
    ...
    if [[ -f /etc/os-release ]]; then
        ...
        if [[ "$ID" == "ubuntu" ]]; then
            if [[ "$VERSION_ID" == "24.04" ]]; then
                log_message "Detected Ubuntu 24.04. Proceeding with installation..."
                return 0
            else
                log_message "WARNING: This script is designed for Ubuntu 24.04, but detected $ID $VERSION_ID."
            ...
            log_message "ERROR: This script is designed for Ubuntu 24.04, but detected $ID $VERSION_ID."
            exit 1
        fi
    else
...
clear
log_message "Starting TuxCare ePortal installation for Ubuntu 24.04..."
echo
...
if apt update && apt install -y --no-install-recommends curl ca-certificates; then
...
if ! check_apt_repo "/etc/apt/sources.list.d/kcare-eportal.list" "https://repo.cloudlinux.com/kcare-eportal/24.04" "TuxCare ePortal"; then
...
# Install with --allow-unauthenticated flag as in original 24.04 script
if apt install -y --no-install-recommends --allow-unauthenticated kcare-eportal; then
...
# Admin password configuration - NORMAL VISIBLE INPUT
log_message "Configuring admin password..."
echo "Your password must contain at least 10 characters."
echo "NOTE: Password will be visible while typing."
while true; do
...
echo ""
echo "Please type 'yes' or 'no' (default: yes)"
...
echo ""
echo "Please type 'yes' or 'no' (default: no)"
